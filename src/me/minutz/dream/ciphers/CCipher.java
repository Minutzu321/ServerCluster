package me.minutz.dream.ciphers;
...
public class CCipher {
    ...
    public static String encrypt(String key, String iv, String data) {
        try {
            if (key.length() < CCipher.CIPHER_KEY_LEN) {
                int numPad = CCipher.CIPHER_KEY_LEN - key.length();
                
                for(int i = 0; i < numPad; i++){
                    key += "0"; //0 pad to len 16 bytes
                }
                
            } else if (key.length() > CCipher.CIPHER_KEY_LEN) {
            ...
            Cipher cipher = Cipher.getInstance(CCipher.CIPHER_NAME);
